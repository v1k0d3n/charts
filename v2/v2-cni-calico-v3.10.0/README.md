...
## Values-supplied JSON Formatting

When using values-supplied JSON, some consideration needs to be made. Some stylistic decisions have been made with these Calico charts. One decision is to provide the JSON structure as a single line value. I will explain the reasons later, but this will require contributors to leverage a couple of tools that can be found online. The first recommended tool is to use [freeformatter.com](https://www.freeformatter.com/json-formatter.html). This can be used to reduce a multi-line JSON config into a single line.

Additionally, you will need to realize that the Calico JSON config *will not* validate out of the box. This is because the value `__CNI_MTU__` is not double-quoted. You can still force the Calico config to properly lint as JSON by double-quoting the value prior to condensing the JSON file into a single string which YAML can then consume. Be sure to remove the quotes when you have reduced the the config, so that Calico node can validate and consume the data provided.

Finally, after you have reduced the JSON config to a single-line string, be sure to double-quote the entire line (beginning to end), and be sure to escape quotes within the string by using a backslash (`\`). Examples are provided below.

**Original Config: v3.4.2**
```
{
